...
if __name__ == "__main__":
    # Inisialisasi sistem dengan optimasi
    recognizer = VowelRecognitionDTW(
        sample_rate=16000,
        use_vad=True,  # Gunakan Voice Activity Detection
        normalize=True  # Gunakan normalisasi CMVN
    )
                        ...
                        if len(vowel_num) >= 2:
                            ...
                            try:
                                ...
                                if vowel_part in vowels:
                                    if vowel_part not in all_files[person]:
                                        all_files[person][vowel_part] = []
                                    
                                    file_path = os.path.join(folder, filename)
                                    all_files[person][vowel_part].append((file_path, file_num))
                            except ValueError:
    ...
    if len(all_closed_results) > 0:
        recognizer.print_confusion_matrix(all_closed_results)
    ...
    with open('results.json', 'w') as f:
        ...
        for key, value in results.items():
            if key != 'overall':
                json_results[key] = {
                    'closed_accuracy': value['closed_accuracy'],
                    'open_accuracy': value['open_accuracy'],
